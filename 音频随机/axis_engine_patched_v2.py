                ...
                info.update(
                    {
                        ...
                        "wav_duration_sec": round(wf.getnframes() / float(max(wf.getframerate(), 1)), 3),
                    }
                )
        except Exception as e:
...
def build_dialogue_speaker_assignment(
    *,
    gender: str,
    speaker_map: Dict[str, Any],
    audio_root_override: Optional[str] = None,
) -> Dict[str, Any]:
    selector_cfg = get_speaker_selector_cfg(audio_root_override=audio_root_override)
    pool = None
    if selector_cfg.get("enabled"):
        try:
            pool = get_reference_audio_pool(audio_root_override=audio_root_override)
        except Exception as e:
    ...
    if pool:
        ...
        if not user_candidates:
            print(f"[WARN] no user candidates for gender={gender}, fallback to all prompts")
    ...
    user_path = speaker_map.get(user_key) or speaker_map.get("User") or speaker_map.get("User_female") or speaker_map.get("User_male")
...
def save_reference_audio_inventory(run_dir: str, pool: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    try:
        ...
        fieldnames = [
            "path", "name", "stem", "ext", "exists", "size_bytes",
            "duration_ms", "duration_sec", "channels", "sample_rate",
            "sample_width", "frame_width", "inspect_error", "wav_valid",
            "wav_channels", "wav_sample_width", "wav_sample_rate",
            "wav_nframes", "wav_duration_sec", "wav_error",
        ]
        with open(csv_path, 'w', encoding='utf-8-sig', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=fieldnames, extrasaction='ignore')
            writer.writeheader()
            for item in inventory:
                writer.writerow(item or {})
        print(f"[REF_AUDIO] inventory_csv={csv_path}")
    except Exception as e:
        print(f"[WARN] inventory csv export failed: {e}")
...
def synthesize_turn(
    ...
    try:
        ...
        speaker_prompt = ((speaker_assignment or {}).get(role) or {}).get("path", "")
        if not speaker_prompt:
            raise RuntimeError(f"missing speaker prompt for role={role}")
...
def export_dataset(
    *,
    ...
    selector_cfg = get_speaker_selector_cfg()
    ref_pool: Optional[Dict[str, Any]] = None
    if selector_cfg.get("enabled"):
        ref_pool = get_reference_audio_pool()
        inv_path = save_reference_audio_inventory(run_dir, ref_pool)
        print(
            f"[CFG] dynamic speaker pool: root={ref_pool.get('root_dir')} "
            f"female={ref_pool.get('summary', {}).get('female_count', 0)} "
            f"male={ref_pool.get('summary', {}).get('male_count', 0)} "
            f"other={ref_pool.get('summary', {}).get('other_count', 0)}"
        )
        if inv_path:
            print(f"[CFG] reference audio inventory saved: {inv_path}")

    if (not isinstance(speaker_map, dict) or not speaker_map) and not selector_cfg.get("enabled"):
        raise RuntimeError(
    ...
    try:
        with open(input_jsonl, "r", encoding="utf-8") as f:
            for line_idx, line in enumerate(f, start=1):
                ...
                speaker_assignment = build_dialogue_speaker_assignment(
                    gender=gender,
                    speaker_map=speaker_map,
                )
                print(
                    f"[SPEAKER] Agent={speaker_assignment['Agent']['name']} | "
                    f"User={speaker_assignment['User']['name']} | mode={speaker_assignment.get('mode')}"
                )
                ...
                for turn_idx, turn in enumerate(conv_list, start=1):
                    ...
                    if with_conversation and conv_turns_dir:
                        ...
                        if wav_all and conv_manifest_fp:
                            ...
                            rec_all = {
                                ...
                                **build_speaker_manifest_fields(speaker_assignment, role),
                            }
                    ...
                    for axis_dir in selected_axes:
                        ...
                        save_speaker_assignment(call_dir, speaker_assignment)
